...
def find_entries(tag=None, dest=None, add_tag=True):
	if tag:
		contents = read_file('../data/dict/cebposdict-nc.txt', strip=True, dict_format=True)
        found_entries = []
		...
		write_file('../data/dict/' + dest, contents=found_entries, add_newline=False)

'''
Corrects NUM
'''
def correct_NUM():
    contents = read_file('../data/dict/old-NUM.txt', strip=True, dict_format=True)
    entries = read_file('../data/dict/cebposdict-nc.txt', strip=True, dict_format=True)

    for content in contents:
        entries[content] = ['NUM']

    write_file('../data/dict/cebposdict-nc.txt', contents=[''], add_newline=False, mode='w')
...
'''
def correct_OTH():
    contents = read_file('../data/dict/new-OTH.txt', strip=True, dict_format=True)
    entries = read_file('../data/dict/cebposdict-nc.txt', strip=True, dict_format=True)
...
def correct_ADV():
    contents = read_file('../data/dict/new-ADV.txt', strip=True, dict_format=True)
    entries = read_file('../data/dict/cebposdict-nc.txt', strip=True, dict_format=True)
...
def remove_function_words():
    ...
    for key, value in function_words.iteritems():
        write_file('../data/function_words/' + key + '.txt', contents=value, add_newline=False, append_newline=True, mode='w')
